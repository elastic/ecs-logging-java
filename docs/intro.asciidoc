...
For more information about ECS, visit the [ECS Reference Documentation]({ecs-ref}/ecs-reference.html).
...
+
As this library adheres to {ecs-ref}/ecs-reference.html[ECS], the Logs UI knows which fields to show
...
It's recommended to use existing {ecs-ref}/ecs-field-reference.html[ECS fields] for MDC values.
